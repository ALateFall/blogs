...
不难想到，目前的沙箱做法中`ptrace`并不是主流的做法，猜测存在缺陷，这里提供一种解决方案：
...
有时候远程的`ld`偏移和本地是不一样的。因此，在条件允许（例如我们有任意地址读）的情况下，我们最好通过`libc`来泄露`ld`地址。
...
有时候还是有用的。使用`Finger`即可，目前我在`IDA8.3`中装了。
